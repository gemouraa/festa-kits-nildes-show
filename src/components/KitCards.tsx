  ...
  return <div ref={cardRef}>
      ...
      transition: {
        ...
        <Card className="h-full overflow-hidden transition-all duration-300 bg-white border-none shadow-lg hover:shadow-xl">
          {/* Imagem do Kit */}
          {kit.image && <div className="relative h-48 sm:h-56 overflow-hidden">
              <img src={kit.image} alt={kit.name} className="w-full h-full object-cover object-center" style={{
            imageRendering: 'crisp-edges'
          }} />
              <motion.div className="absolute top-2 right-2" initial={{
            rotate: 0
          }} animate={{
            rotate: [0, 15, -15, 0]
          }} transition={{
          ...
          <CardContent className="p-4 sm:p-6">
            <ul className="space-y-2 sm:space-y-3 font-poppins">
              {kit.items.map((item: string, i: number) => <motion.li key={i} initial={{
              opacity: 0,
              x: -10
            }} animate={isInView ? {
              opacity: 1,
          ...
          <CardFooter className="p-4 sm:p-6">
            <Button onClick={() => onSelectKit(kit)} className="w-full gap-2 text-white transition-all duration-300 shadow-md bg-green-600 hover:bg-green-700 hover:shadow-lg rounded-full font-poppins text-sm sm:text-base py-2 sm:py-3">
              <WhatsappIcon className="w-4 h-4 sm:w-5 sm:h-5" />
              <span className="relative">
                <span className="relative z-10">Escolher este kit</span>
                <motion.span className="absolute bottom-0 left-0 w-full h-1 bg-white/40" initial={{
                width: 0
              }} whileHover={{
                width: '100%'
              }} transition={{
...
const KitCards: React.FC<KitCardsProps> = ({
  ...
  return <div ref={containerRef}>
      <motion.div className="grid gap-6 sm:gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" initial={{
      opacity: 0
    }} animate={isInView ? {
      opacity: 1
    } : {
      ...
      staggerChildren: 0.1
        ...
        {kits.map((kit, index) => <KitCard key={kit.id} kit={kit} index={index} onSelectKit={onSelectKit} />)}
      </motion.div>
    </div>;
};
