...
const KitCard: React.FC<KitCardProps> = ({ kit, index, onSelectKit }) => {
        ...
        <Card className="h-full overflow-hidden transition-all duration-300 bg-white border-none shadow-lg hover:shadow-xl">
          ...
          <CardHeader className="relative bg-gradient-to-r from-sky-400 to-purple-400 text-center">
            ...
            <div className="text-center">
              <span className="text-2xl font-bold text-white">R$ {kit.price}</span>
              {kit.originalPrice && (
                <span className="text-sm text-white/80 line-through ml-2">
                  R$ {kit.originalPrice}
                </span>
              )}
            </div>
...
const KitCards: React.FC<KitCardsProps> = ({ onSelectKit }) => {
